...
from src.data.multi_goal_gridworld import (
    MultiGoalDataConfig,
    MultiGoalSequenceImitationDataset,
    generate_sequence_multigoal_imitation_samples,
)
...
@torch.no_grad()
def _evaluate_multigoal_rollout(model, data_config: MultiGoalDataConfig, number_of_episodes: int, seed_value: int, sequence_length: int, device):
    ...
    for _ in range(number_of_episodes):
        ...
        while not done and steps_taken < environment.max_steps:
            ...
            full_observation, _, done, _ = environment.step(action)
            steps_taken += 1

        goals_completed = environment.current_goal_index
        if environment.agent_pos == environment.target_pos:
            goals_completed += 1

        completion_ratio = goals_completed / max(1, data_config.number_of_goals)
        average_goal_completion.append(completion_ratio)
        step_counts.append(steps_taken)
        if goals_completed >= data_config.number_of_goals:
            full_successful_episodes += 1

    return {
        "full_success_rate": full_successful_episodes / max(1, number_of_episodes),
        "mean_goal_completion_ratio": float(np.mean(average_goal_completion)),
        "average_steps": float(np.mean(step_counts)),
    }
...
def main():
    ...
    train_x, train_y = generate_sequence_multigoal_imitation_samples(
        int(split_cfg["train_episodes"]), data_cfg, seed_value, sequence_length
    )
    val_x, val_y = generate_sequence_multigoal_imitation_samples(
        int(split_cfg["val_episodes"]), data_cfg, seed_value + 1, sequence_length
    )
    test_x, test_y = generate_sequence_multigoal_imitation_samples(
        int(split_cfg["test_episodes"]), data_cfg, seed_value + 2, sequence_length
    )

    batch_size = int(config["train"]["batch_size"])
    train_loader = DataLoader(MultiGoalSequenceImitationDataset(train_x, train_y), batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(MultiGoalSequenceImitationDataset(val_x, val_y), batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(MultiGoalSequenceImitationDataset(test_x, test_y), batch_size=batch_size, shuffle=False)

    ood_cfg = config["data"].get("ood", {})
    ood_enabled = bool(ood_cfg.get("enabled", False))
    if ood_enabled:
        ood_data_cfg = MultiGoalDataConfig(
            ...
            number_of_goals=int(ood_cfg.get("number_of_goals", config["data"]["number_of_goals"])),
            observation_crop_size=int(config["data"]["observation_crop_size"]),
            episode_horizon_min=int(config["data"]["episode_horizon_min"]),
            episode_horizon_max=int(config["data"]["episode_horizon_max"]),
        )
        ood_x, ood_y = generate_sequence_multigoal_imitation_samples(
            int(ood_cfg.get("test_episodes", split_cfg["test_episodes"])), ood_data_cfg, seed_value + 20, sequence_length
        )
        ood_loader = DataLoader(MultiGoalSequenceImitationDataset(ood_x, ood_y), batch_size=batch_size, shuffle=False)
    else:
        ood_data_cfg = None
        ood_loader = None
    ...
    print(f"Number of goals (train): {data_cfg.number_of_goals}")
    if ood_enabled:
        print(
            f"OOD test samples: {len(ood_y)} "
            f"(grid={ood_data_cfg.grid_size}, obstacles={ood_data_cfg.obstacle_count}, goals={ood_data_cfg.number_of_goals})"
        )
    ...
    iid_rollout_metrics = _evaluate_multigoal_rollout(
    ...
    metrics = {
        ...
        "lstm_multigoal": {
            ...
            "iid_full_success_rate": iid_rollout_metrics["full_success_rate"],
            "iid_mean_goal_completion_ratio": iid_rollout_metrics["mean_goal_completion_ratio"],
            "iid_test_avg_steps": iid_rollout_metrics["average_steps"],
            "ood_test_action_accuracy": ood_test_action_accuracy,
            "ood_full_success_rate": None if ood_rollout_metrics is None else ood_rollout_metrics["full_success_rate"],
            "ood_mean_goal_completion_ratio": None if ood_rollout_metrics is None else ood_rollout_metrics["mean_goal_completion_ratio"],
            "ood_test_avg_steps": None if ood_rollout_metrics is None else ood_rollout_metrics["average_steps"],
            "history": history,
        },
    ...
    print("\n=== Summary ===")
    print(
        f"IID action acc: {iid_test_action_accuracy:.4f}, "
        f"full success: {iid_rollout_metrics['full_success_rate']:.4f}, "
        f"goal completion: {iid_rollout_metrics['mean_goal_completion_ratio']:.4f}"
    )
    if ood_test_action_accuracy is not None:
        print(
            f"OOD action acc: {ood_test_action_accuracy:.4f}, "
            f"full success: {ood_rollout_metrics['full_success_rate']:.4f}, "
            f"goal completion: {ood_rollout_metrics['mean_goal_completion_ratio']:.4f}"
        )
