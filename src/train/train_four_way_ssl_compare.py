...
from src.models.predictive_ssl_policy import PredictiveSslPolicyModel
...
def _parameter_memory_mb(model: nn.Module):
    trainable_parameters = sum(parameter.numel() for parameter in model.parameters())
    return float(trainable_parameters * 4 / (1024 ** 2))
...
def _train_rotation_ssl_epoch(model, loader, optimizer, criterion, device, augmentation_config: dict | None = None):
    ...
    for observation_batch, _ in loader:
        observation_batch = observation_batch.to(device)
        augmented_batch = _augment_for_ssl(observation_batch, augmentation_config)
        ...
        loss_value.backward()
        optimizer.step()
        total_loss += loss_value.item() * observation_batch.size(0)
        total_count += observation_batch.size(0)
    return total_loss / max(1, total_count)
...
def _train_contrastive_ssl_epoch(
    ...
    for observation_batch, _ in loader:
        observation_batch = observation_batch.to(device)
        view_1 = _augment_for_ssl(observation_batch, augmentation_config)
        view_2 = _augment_for_ssl(observation_batch, augmentation_config)
...
def _train_predictive_ssl_epoch(model, loader, optimizer, criterion, device):
    model.train()
    total_loss = 0.0
    total_count = 0
    for observation_batch, _ in loader:
        observation_batch = observation_batch.to(device)
        with torch.no_grad():
            next_observation_batch = torch.roll(observation_batch, shifts=-1, dims=0)
            target_next_embedding = model.encode(next_observation_batch).detach()
        predicted_next_embedding = model.forward_predict_next_embedding(observation_batch)
        loss_value = criterion(predicted_next_embedding, target_next_embedding)
        optimizer.zero_grad()
        loss_value.backward()
        optimizer.step()
        total_loss += loss_value.item() * observation_batch.size(0)
...
def _run_linear_probe_phase(model, train_loader, val_loader, test_loader, device, epochs: int, learning_rate: float):
    for parameter in model.encoder.parameters():
        parameter.requires_grad = False
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.action_head.parameters(), lr=learning_rate)
    best_val = 0.0
    for _ in range(epochs):
        _train_action_epoch_ssl_model(model, train_loader, optimizer, criterion, device)
...
def main():
    ...
    parser.add_argument("--config", type=str, default="experiments/configs/four_way_ssl_compare_quickcheck.json")
    ...
    for _ in range(int(config["train"]["supervised_epochs"])):
        _train_supervised_epoch(supervised_model, train_loader, supervised_optimizer, criterion, device)
        supervised_best_val = max(supervised_best_val, _evaluate_action_accuracy_supervised(supervised_model, val_loader, device))
    ...
    for _ in range(int(config["train"]["rotation_ssl_pretrain_epochs"])):
        _train_rotation_ssl_epoch(rotation_model, train_loader, rotation_ssl_optimizer, criterion, device, augmentation_config)
    rotation_lp_val, rotation_lp_iid = _run_linear_probe_phase(
        rotation_model, train_loader, val_loader, test_loader, device, int(config["train"]["linear_probe_epochs"]), float(config["train"]["learning_rate_linear_probe"])
    )
    rotation_best_val, rotation_iid_acc, _ = _run_fine_tune_phase(
        rotation_model, train_loader, val_loader, test_loader, device,
        int(config["train"]["fine_tune_epochs"]),
        float(config["train"]["learning_rate_fine_tune_encoder"]),
        float(config["train"]["learning_rate_fine_tune_head"]),
    )
    ...
    rotation_ood_acc = _evaluate_action_accuracy_ssl_model(rotation_model, ood_loader, device) if ood_enabled else None
    rotation_ood_success, rotation_ood_steps = _evaluate_rollout(rotation_model.forward_action, ood_data_cfg, int(ood_cfg["test_episodes"]), seed_value + 30, device) if ood_enabled else (None, None)
    ...
    for _ in range(int(config["train"]["contrastive_ssl_pretrain_epochs"])):
        _train_contrastive_ssl_epoch(
            contrastive_model,
            train_loader,
    ...
    contrastive_lp_val, contrastive_lp_iid = _run_linear_probe_phase(
        contrastive_model, train_loader, val_loader, test_loader, device, int(config["train"]["linear_probe_epochs"]), float(config["train"]["learning_rate_linear_probe"])
    )
    contrastive_best_val, contrastive_iid_acc, _ = _run_fine_tune_phase(
        contrastive_model, train_loader, val_loader, test_loader, device,
        int(config["train"]["fine_tune_epochs"]),
        float(config["train"]["learning_rate_fine_tune_encoder"]),
        float(config["train"]["learning_rate_fine_tune_head"]),
    )
    ...
    contrastive_ood_acc = _evaluate_action_accuracy_ssl_model(contrastive_model, ood_loader, device) if ood_enabled else None
    contrastive_ood_success, contrastive_ood_steps = _evaluate_rollout(contrastive_model.forward_action, ood_data_cfg, int(ood_cfg["test_episodes"]), seed_value + 30, device) if ood_enabled else (None, None)

    # predictive
    predictive_model = PredictiveSslPolicyModel(
        input_channels=3,
        embedding_size=int(config["model"]["embedding_size"]),
        number_of_actions=4,
    ).to(device)
    predictive_ssl_optimizer = torch.optim.Adam(
        list(predictive_model.encoder.parameters()) + list(predictive_model.predictor_head.parameters()),
        lr=float(config["train"]["learning_rate_ssl"]),
    )
    predictive_criterion = nn.MSELoss()
    for _ in range(int(config["train"]["predictive_ssl_pretrain_epochs"])):
        _train_predictive_ssl_epoch(predictive_model, train_loader, predictive_ssl_optimizer, predictive_criterion, device)
    predictive_lp_val, predictive_lp_iid = _run_linear_probe_phase(
        predictive_model, train_loader, val_loader, test_loader, device, int(config["train"]["linear_probe_epochs"]), float(config["train"]["learning_rate_linear_probe"])
    )
    predictive_best_val, predictive_iid_acc, _ = _run_fine_tune_phase(
        predictive_model, train_loader, val_loader, test_loader, device,
        int(config["train"]["fine_tune_epochs"]),
        float(config["train"]["learning_rate_fine_tune_encoder"]),
        float(config["train"]["learning_rate_fine_tune_head"]),
    )
    predictive_iid_success, predictive_iid_steps = _evaluate_rollout(predictive_model.forward_action, data_cfg, int(split_cfg["test_episodes"]), seed_value + 3, device)
    predictive_ood_acc = _evaluate_action_accuracy_ssl_model(predictive_model, ood_loader, device) if ood_enabled else None
    predictive_ood_success, predictive_ood_steps = _evaluate_rollout(predictive_model.forward_action, ood_data_cfg, int(ood_cfg["test_episodes"]), seed_value + 30, device) if ood_enabled else (None, None)
    ...
    metrics = {
        ...
        "compute": {
            ...
            "parameter_count": {
                "supervised": _count_parameters(supervised_model),
                "rotation": _count_parameters(rotation_model),
                "contrastive": _count_parameters(contrastive_model),
                "predictive": _count_parameters(predictive_model),
            },
            "parameter_memory_mb": {
                "supervised": _parameter_memory_mb(supervised_model),
                "rotation": _parameter_memory_mb(rotation_model),
                "contrastive": _parameter_memory_mb(contrastive_model),
                "predictive": _parameter_memory_mb(predictive_model),
            },
        },
        "supervised": {
        ...
        "rotation_ssl_fine_tune": {
            "linear_probe_val_action_accuracy": rotation_lp_val,
            "linear_probe_iid_action_accuracy": rotation_lp_iid,
        ...
        "contrastive_ssl_fine_tune": {
            "linear_probe_val_action_accuracy": contrastive_lp_val,
            "linear_probe_iid_action_accuracy": contrastive_lp_iid,
        ...
        "predictive_ssl_fine_tune": {
            "linear_probe_val_action_accuracy": predictive_lp_val,
            "linear_probe_iid_action_accuracy": predictive_lp_iid,
            "best_validation_action_accuracy": predictive_best_val,
            "iid_test_action_accuracy": predictive_iid_acc,
            "iid_test_success_rate": predictive_iid_success,
            "iid_test_avg_steps_to_goal": predictive_iid_steps,
            "ood_test_action_accuracy": predictive_ood_acc,
            "ood_test_success_rate": predictive_ood_success,
            "ood_test_avg_steps_to_goal": predictive_ood_steps,
        },
    }
    ...
    torch.save(predictive_model.state_dict(), run_directory / "predictive_ssl_model_state_dict.pth")

    table_path = repository_root / "reports" / "tables" / f"{config['experiment_name']}_summary.md"
    table_text = (
        ...
        "|---|---:|---:|---:|---:|---:|\n"
        f"| Supervised end-to-end | - | {supervised_iid_acc:.4f} | {supervised_ood_acc if supervised_ood_acc is not None else float('nan'):.4f} | {supervised_iid_success:.4f} | {supervised_ood_success if supervised_ood_success is not None else float('nan'):.4f} |\n"
        f"| Rotation SSL + Fine-tune | {rotation_lp_iid:.4f} | {rotation_iid_acc:.4f} | {rotation_ood_acc if rotation_ood_acc is not None else float('nan'):.4f} | {rotation_iid_success:.4f} | {rotation_ood_success if rotation_ood_success is not None else float('nan'):.4f} |\n"
        f"| Contrastive SSL + Fine-tune | {contrastive_lp_iid:.4f} | {contrastive_iid_acc:.4f} | {contrastive_ood_acc if contrastive_ood_acc is not None else float('nan'):.4f} | {contrastive_iid_success:.4f} | {contrastive_ood_success if contrastive_ood_success is not None else float('nan'):.4f} |\n"
        f"| Predictive SSL + Fine-tune | {predictive_lp_iid:.4f} | {predictive_iid_acc:.4f} | {predictive_ood_acc if predictive_ood_acc is not None else float('nan'):.4f} | {predictive_iid_success:.4f} | {predictive_ood_success if predictive_ood_success is not None else float('nan'):.4f} |\n"
    )
    with open(table_path, "w", encoding="utf-8") as table_file:
        table_file.write(table_text)

    print("Saved run artifacts to:", run_directory)
